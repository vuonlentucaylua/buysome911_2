 ...
 * @since 1.0.3
 */
class Deep_Core_Recently_Active_Widget extends WP_Widget {
	...
	public function __construct() {
		$name        = _x( 'Webnus (BuddyPress) Recently Active Members', 'widget name', 'deep' );
		$description = __( 'Profile photos of recently active members', 'deep' );
		parent::__construct( false, $name, array(
			'description'                 => $description,
			'classname'                   => 'widget_Deep_core_recently_active_widget buddypress widget',
	...
	public function widget( $args, $instance ) {
		...
		<?php if ( bp_has_members( $members_args ) ) : ?>
			...
			<div class="wn-avatar-block owl-carousel owl-theme">
		...
		<?php else: ?>
			...
			<div class="widget-error">
				<?php esc_html_e( 'There are no recently active members', 'deep' ); ?>
			</div>
	...
	public function form( $instance ) {
		...
		<p>
			<label for="<?php echo '' . $this->get_field_id( 'max_members' ); ?>">
				<?php esc_html_e( 'Max members to show:', 'deep' ); ?>
	...
	public function parse_settings( $instance = array() ) {
		return bp_parse_args( $instance, array(
			'title' 	     => __( 'Recently Active Members', 'deep' ),
			'max_members' 	 => 15,
	...
	function register_Deep_Core_Recently_Active_Widget_widget() {
		register_widget( 'Deep_Core_Recently_Active_Widget' );
	}
	add_action( 'widgets_init', 'register_Deep_Core_Recently_Active_Widget_widget' );
