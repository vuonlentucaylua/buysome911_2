...
?>
<?php
define('Z_IMAGE_PLACEHOLDER', DEEP_ASSETS_URL ."/images/no_speaker.jpg");
...
function z_add_texonomy_field() {
	...
	echo '<div class="form-field">
		<label for="taxonomy_image">' . __('Image', 'deep') . '</label>
		<input type="text" name="taxonomy_image" id="taxonomy_image" value="" />
		<br/>
		<button class="z_upload_image_button button">' . __('Upload/Add image', 'deep') . '</button>
	</div>'.z_script();
}
...
function z_edit_texonomy_field($taxonomy) {
	...
	echo '<tr class="form-field">
		<th scope="row" valign="top"><label for="taxonomy_image">' . __('Image', 'deep') . '</label></th>
		...
		<button class="z_upload_image_button button">' . __('Upload/Add image', 'deep') . '</button>
		<button class="z_remove_image_button button">' . __('Remove image', 'deep') . '</button>
		</td>
	</tr>'.z_script();
}
...
function z_save_taxonomy_image($term_id) {
	$tax_img = esc_url_raw( $_POST['taxonomy_image'] );
    if( isset( $tax_img ) )
        update_option( 'z_taxonomy_image'.$term_id, $tax_img );
}
...
function z_get_attachment_id_by_url($image_src) {
    ...
    $id = $wpdb->get_var( $wpdb->prepare( $query, get_the_ID() ) );
...
function z_taxonomy_columns( $columns ) {
	...
	$new_columns['thumb'] = __('Image', 'deep');
...
function z_taxonomy_column( $columns, $column, $id ) {
	if ( $column == 'thumb' )
		$columns = '<span><img src="' . z_taxonomy_image_url($id, NULL, TRUE) . '" alt="' . __('Thumbnail', 'deep') . '" class="wp-post-image" /></span>';
